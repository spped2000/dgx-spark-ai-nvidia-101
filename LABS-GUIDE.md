...
เปิด **http://localhost:8888** → login (`studentXX` / `dgxspark2026`) → ได้ container ส่วนตัว → โน้ตบุ๊ก `labs/00_welcome.ipynb` เด้งขึ้น → กด `Shift+Enter` ทีละเซลล์ (ไม่ต้องเขียนโค้ด)
