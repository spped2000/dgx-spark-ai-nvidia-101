...
- `05_skills_verified_agent` (LAB 6 bonus) = **agent + NVIDIA-verified skill** (ผู้เรียนทำเอง)
...
- 🎯 **เอาไว้ทำอะไร:** เชื่อม **NVIDIA Agent Skills** (คลังความสามารถที่ NVIDIA เซ็นรับรอง) + agent + signing(security) — skill อัป capability ของ agent และต้อง **verify ก่อนใช้**
- 🔧 **Tools:** vLLM (`:8000`) · repo `NVIDIA/skills` (mount `~/nvidia-skills`) · เชื่อม OpenShell/NemoClaw (production stack ที่รวม agent+sandbox+skills)
- 🧠 **Model:** `nemotron-nano`
- 🔄 **Flow:** เลือก skill (cuDF/cuOpt) → ดู skill-card → **ตรวจ OMS signature + NVIDIA root cert** → ถาม agent ไม่มี skill vs มี skill → เทียบ (เฉพาะทางขึ้น)
- 🎬 **Scenario:** "skill = ความสามารถพกพา + เซ็นรับรอง; ใส่ให้ agent → ตอบเรื่อง cuDF/cuOpt ได้ถูกต้องขึ้น (verified = เชื่อถือได้ ↔ LAB 5)"
- ▶️ **เรียกใช้:** ผู้เรียนเปิด `labs/05_skills_verified_agent.ipynb` · ติดตั้ง skill จริง: `npx skills add nvidia/skills`
